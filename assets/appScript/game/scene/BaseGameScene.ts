...
import BaseMapController from '../element/map/BaseMapController';
...
export default class BaseGameScene extends BaseScene {
    ...
    protected mapController: BaseMapController = null; // 地图控制器
    ...
    OnLoad() {
        ...
        this.mapController = map.getComponent('BaseMapController');
    ...
    protected onLoadPlayerData(playsData: any[]) {
        ...
        this.mapController.initMap(playsData);
        // 通过roomData创建游戏视图
        const snakeControllers = this.mapController.getSnakeController();
        snakeControllers.forEach(snakeController => {
            const gameId = snakeController.gameId;
    ...
    listener() {
        ...
        CmdModule.on(CmdModule.EventType.receiveCmd, (cmd) => {
            this.tickLogic(cmd);
        });
        // 当蛇杀死其他蛇时
